...
class processMember extends Member
{
	...
	function leave()
	{
		...
		$leftMessage=l_t("Has abandonado <strong>%s</strong> antes de que empezara y se te ha devuelto tu apuesta de <strong>%s</strong>.",
		...
		else
		{
			...
			$Game->Members->sendExcept($this,'No',l_t("<strong>%s</strong> abandonó la partida.",$name));
		}
	...
	function awardPoints($awardedPoints)
	{
		...
		if ( $this->points > 100 )
		{
			...
			if ( $position > $oldPosition )
				$this->send('No','No',l_t("Tu victoria en esta partida te ha hecho avanzar en el ranking global desde el puesto #%s al #%s!",$oldPosition,$position));
		}
	...
	function setSurvived($winnings)
	{
		...
		if ( $Game->potType == 'Winner-takes-all' )
		{
			$but="";
			if($winnings)
				$but = l_t(" (pero recibes %s de vuelta para completar tus 100 iniciales)",$winnings);

			$this->send('No','No',l_t("La partida ha terminado: has sobrevivido hasta el final, pero como era una partida a Todo-o-Nada no se te devuelve ningún ningún puntos%s. ¡Más suerte la próxima vez!",$but));
		}
		else
		{
			$this->send('No','No',l_t("La partida ha terminado: has sobrevivido y recibes %s",$winnings.' '.libHTML::points()));
		}
	}
	...
	function setLeft()
	{
		...
		$this->send('No','No',l_t("Tu nación ha estado inactiva y ha caído en Desorden Civil. Ahora la puede tomar cualquiera, salvo que tú la recuperes antes."));
		$this->Game->Members->sendExcept($this,'No',l_t('%s ha entrado en Desorden Civil.',$this->Game->Variant->countries[$this->countryID-1]));
	}
	...
	function setAbandoned()
	{
		...
		$but="";
		if($refundedPoints)
			$but=l_t("Has recibido %s para completar el mínimo de 100. ",$refundedPoints.' '.libHTML::points());

		$this->send('No','No',l_t("Debido a la inactividad la partida ha sido abandonada y borrada. %s¡Más suerte la próxima vez!",$but));
	}
	...
	function setNotEnoughPlayers()
	{
		...
		// No need to set status, we're about to be deleted
		$this->send('No','No',
				l_t("Esta partida ha sido cancelada y se te ha devuelto la apuesta de %s: ".
				"La partida no ha alcanzado el mínimo de %s requeridos. Inenta encontrar jugadores antes de ".
				"crear una partida, crea partidas con la fase más larga, o únete a una partida ya existente.",
	...
	function setCancelled()
	{
		...
		$this->setStatus('Survived');
		$this->send('No', 'No',
				l_t("Esta partida ha sido cancelada y se te ha devuelto la apuesta de %s.",$refundedPoints.libHTML::points()));
	}
...
//			$but=l_t(", but you have been refunded %s to make up your starting 100",$refundedPoints);
		...
		$this->send('No','No',l_t("Se han hecho cargo de tu país en Desorden Civil, así que has perdido ".
			"tu apuesta%s. ¡Ten cuidado la próxima vez!",$but));
	}
	...
	function setResigned()
	{
		...
		$but="";
		if($refundedPoints)
			$but=l_t("Se te han devuelto %s para completar tus 100 iniciales. ",$refundedPoints);

		$this->send('No','No',l_t("La partida ha terminado y tu país ha sobrevivido, pero estaba en Desorden Civil. %s".
			"¡Ten cuidado la próxima vez!",$but));
	}
	...
	function setDefeated()
	{
		...
		$but="";
		if($refundedPoints)
			$but=l_t(", pero se te han devuelto %s para completar los 100 iniciales",$refundedPoints);

		$this->send('No','No',l_t("Te han derrotado y has perdido tu apuesta%s. ¡Suerte la próxima!",$but));
		$this->Game->Members->sendExcept($this,'No',l_t('%s ha sido derrotado.',$this->Game->Variant->countries[$this->countryID-1]));
	}
	...
	function setDrawn($winnings)
	{
		...
		$this->send('No','No',l_t("Has empatado con tus rivales y sobrevivido. ".
			"¡Ganas %s, una parte proporcional de la apuesta!",$winnings." ".libHTML::points()));
	}
	...
	function setWon($winnings)
	{
		...
		$this->send('No','No',l_t("¡Felicidades! ¡Has ganado la partida y obtienes %s!",$winnings." ".libHTML::points()));
		$this->Game->Members->sendExcept($this,'No',l_t('%s ha ganado la partida y se lleva %s!',$this->Game->Variant->countries[$this->countryID-1],$winnings." ".libHTML::points()));
	}
