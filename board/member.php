	...
	public function toggleVote($voteName)
	{
		...
		else
		{
			...
			if ($count == 1)
			{
				...
				$msg = $this->country.l_t(' voted for a ').'<b>'.l_t($voteText).'</b>. ';
				if ($voteText == 'Draw')
					$msg .= l_t('If everyone votes Draw the game will end and the points are split equally among all the surviving players, regardless of how many supply centers each player has.');
				if ($voteText == 'Pause')
					$msg .= l_t('If everyone votes Pause the game stop and wait till everybody votes Unpause. Please consider backing this.');
				if ($voteText == 'Unpause')
					$msg .= l_t('If everyone votes Unpause the game will continoue. Please consider backing this.');
				if ($voteText == 'Cancel')
					$msg .= l_t('If everyone votes Cancel all points will be refunded and the game will be deleted from the database.');
				if ($voteText == 'Extend')
					$msg .= l_t('If 2/3 of the active players vote Extend the the current phase will be extend by 4 days. Please consider backing this. If the majority is not reached by "').$this->Game->Variant->turnAsDate($this->Game->turn + 2).l_t('" the votes will be cleared.');
				if ($voteText == 'Concede')
					$msg .= l_t('If everyone (but one) votes Concede the game will end and the player _not_ voting Conceede will get all the points. Everybody else will get a defeat.');			
				libGameMessage::send(0, 'GameMaster', l_t($msg) , $this->Game->id);
			}
		}
