		...
		// WYSIWYG for all compatible browsers
			...
			if (!tinyMCE.baseURL) {
				...
				for (var i = 0; i < elements.length; i++) {
					if (elements[i].src && (elements[i].src.indexOf('wysiwyg') != -1)) {
...
// to be able to handle more than one simple textarea	
	...
	setupContent : function(editor_id) {
		...
		if (inst.getBody().innerHTML.match(/<blockquote quotedata=.*?<\/blockquote>/)) {
			...
			if (!tinyMCE.isSafari && !tinyMCE.isOpera) { 
				...
				if (elm) {
					...
					if (size == '') {
						...
						//var sizes = new Array('', 'X-Small', 'Small', 'Medium', 'Large', 'X-Large', 'XX-Large');
...
TinyMCE_Control.prototype = {
	...
	_onAdd : function(replace_element, form_element_name, target_document) {
		...
		tinyMCE.createVideoSelect(this.editorId);
	...
	execCommand : function(command, user_interface, value, selectedText) {
		...
		switch (command.toLowerCase()) {
			...
			case "mcewysiwygeditor":
				...
				// Use original href (mybb_href) instead of the browser-inserted href
				var aElements = this.getDoc().getElementsByTagName('A');
				for (var i = 0; i < aElements.length; i++) {
					if (href = aElements[i].getAttribute('mybb_href')) {
						aElements[i].setAttribute("href", href);
					}
				}
				
				// Use original src (mybb_src) instead of the browser-inserted src
				var imgElements = this.getDoc().getElementsByTagName('IMG');
				for (var i = 0; i < imgElements.length; i++) {
					if (src = imgElements[i].getAttribute('mybb_src')) {
			...
			case "mceimage":
				if (this.editorIsActive) {
					...
					if (img) {
						src = tinyMCE.getAttrib(img, 'mybb_src');
			...
			case "mcevideotag":
				if (this.editorIsActive) {
					this.insertContent('[video='+value+']', '[/video]');
					tinyMCE.triggerNodeChange();
					tinyMCE.execCommand("mceAddUndoLevel");	
				}
				else tinyMCE.simpleExecCommand('video', this.editorId+'_codeview', value);
				return true;
...
// TODO: Insert HTML on cursor position					
            ...
            case "mceinsertcontent2":
			alert('oO');
				if (this.editorIsActive) 
				{
				    value = tinyMCE.bbCodeToHTML(value);
				}
				alert(value);
...
//-------------------------- CLEAN PASTE -------------------------------\\	
	...
	cleanupHTML : function() {
		...
		// Add absolute src and add mybb_src to all images
		content = content.replace(/<img(.*?)src="(.*?)"/gi, function(thisMatch, attributes, src) {
			if (!src.match(/^http/)) src = tinyMCE.settings['base_href'] + src;
			return '<img'+ attributes +'src="' + src + '" mybb_src='+src;
		});
		
		// Add absolute href and add mybb_href to all links
		content = content.replace(/<a(.*?)href="(.*?)"/gi, function(thisMatch, attributes, href) {
			...
			return '<a'+ attributes +'href="' + href + '" mybb_href='+href;
		});
		...
		for (var smileyCode in smilies) {
			...
			if (content.match(smileyRegex)) {
				...
				var html = '<img src="' + url + '" mybb_src="' + url + '" alt="' + smileyCode + '" />';							
...
function getEditorTemplate (settings, editorId) {
		...
		// Resizer icon
		template['html'] += '<div class="border mceResizeIconRow"><div class="container-1 editorFooter">';
...
TinyMCE_Engine.prototype.getToolbarElements = function(editorId) {
	...
	*/
	...
	fontFamilyOptions = '<option value="">' + language['fontFamily.default'] + '</option>'+
		...
		'<option style="font-family:Chicago, Impact, Compacta, sans-serif" value="Chicago">Chicago</option>'+
		'<option style="font-family:\'Comic Sans MS\', sans-serif" value="Comic Sans MS">Comic Sans MS</option>'+
		'<option style="font-family:\'Courier New\', Courier, mono" value="Courier New">Courier New</option>'+
		'<option style="font-family:Geneva, Arial, Helvetica, sans-serif" value="Geneva">Geneva</option>'+
		'<option style="font-family:Georgia, \'Times New Roman\', Times, serif" value="Georgia">Georgia</option>'+
		'<option style="font-family:Helvetica, Verdana, sans-serif" value="Helvetica">Helvetica</option>'+
		'<option style="font-family:Impact, Compacta, Chicago, sans-serif" value="Impact">Impact</option>'+
		'<option style="font-family:\'Lucida Sans\', Monaco, Geneva, sans-serif" value="Lucida Sans">Lucida Sans</option>'+
		'<option style="font-family:Tahoma, Arial, Helvetica, sans-serif" value="Tahoma">Tahoma</option>'+
		'<option style="font-family:\'Times New Roman\', Times, Georgia, serif" value="Times New Roman">Times New Roman</option>'+
		'<option style="font-family:\'Trebuchet MS\', Arial, sans-serif" value="Trebuchet MS">Trebuchet MS</option>'+
		'<option style="font-family:Verdana, Helvetica, sans-serif" value="Verdana">Verdana</option>';
	
	// Font sizes options
	fontSizeOptions = '<option value="0">' + language['fontsize.default'] + '</option>'+
		'<option style="font-size:8pt" value="1">X-Small</option>'+
		'<option style="font-size:10pt" value="2">Small</option>'+
		'<option style="font-size:12pt" value="3">Medium</option>'+
		'<option style="font-size:14pt" value="4">Large</option>'+
		'<option style="font-size:18pt" value="5">X-Large</option>'+
		'<option style="font-size:24pt" value="6">XX-Large</option>';
			...
			for (var code in coreBBCodes) {
				...
				if (code == toolbarElement) {
					...
					if (code.match(/^(b|i|u|s|quote|img)$/) || bbCodeElement['sourceCode']) {
						...
						toolbarHTML += '<li id="' + editorId + '_video_li">';
						toolbarHTML += '<a id="' + editorId + '_video" href="javascript:void(0)" onmousedown="return false;" class="" target="_self">'; 
						toolbarHTML += '<img src="' + this.settings['imageURL'] +  bbCodeElement['icon'] + '" title="' + language['video.desc'] + '" />';
						toolbarHTML += '</a>';
						toolbarHTML += '</li>';
...
TinyMCE_Engine.prototype.loadSimpleTextarea = function() {
	...
	tinyMCE.createVideoSelect(this.simpleAreaID, true);
...
TinyMCE_Engine.prototype.simpleExecCommand = function(command, textareaID, attribut) {
	...
	if (command == 'FontSize') {
	   // Array for converting "size" (1-7) to pt (8-36)
		...
		fontPointSizes[1] = 'x-small';
		fontPointSizes[2] = 'small';
		fontPointSizes[3] = 'medium';
		fontPointSizes[4] = 'large';
		fontPointSizes[5] = 'x-large';
		fontPointSizes[6] = 'xx-large';
...
/* Inserts quotes */
TinyMCE_Engine.prototype.insertQuotes = function(quotes) {
	if (!tinyMCE.isSimpleTextarea) {
		var inst = tinyMCE.selectedInstance;
		if (inst.editorIsActive) {
		    var editorID = inst.editorId;
		    var quotes = tinyMCE.bbCodeToHTML(quotes);
    		tinyMCE.execInstanceCommand(inst.editorId, "mceInsertContent", false, quotes);
		}
		else { tinyMCE.simpleExecCommand('mceText', inst.editorId+'_codeview', quotes);
		}
	}
	else tinyMCE.simpleExecCommand('mceText', 'text', quotes);
return;
var inst = tinyMCE.selectedInstance;
var html = '<b>haha</b>';
tinyMCE.execInstanceCommand(inst.editorId, "mceInsertContent", false, quotes);
return;
tinyMCE.execCommand('mceInsertContent2', false, 'wooooto');
alert('haaaha');
	if (!tinyMCE.isSimpleTextarea) {
		var inst = tinyMCE.selectedInstance;
		if (inst.editorIsActive) tinyMCE.execCommand("mceInsertContent", false, "xD");
		else tinyMCE.simpleExecCommand('smiley', inst.editorId+'_codeview', ' ' + code + ' ');
	}
	else tinyMCE.simpleExecCommand('smiley', 'text', ' ' + code + ' ');
};
...
TinyMCE_Engine.prototype.insertAttachment = function (attachmentID) {
	if (!tinyMCE.isSimpleTextarea) {
		...
		if (inst.editorIsActive) {
			...
			else tag = '[attachment=' + attachmentID + ']';
...
TinyMCE_Engine.prototype.insertImage = function(editorID, src, alt, title) {
	...
	var html = '&nbsp;<img src="' + src + '" mybb_src="' + src + '" alt="' + alt + '" title="' + title + '"  />&nbsp;';
...
TinyMCE_Engine.prototype.bbCodeToHTML = function (bbCode, switchView) { 
	...
	html = html.replace(/\[size=["']?([^"'\]]+)["']?\]/gi, function(thisMatch, size) {
		...
		fontPointSizes[24] = 6;
		fontPointSizes[36] = 7;
		fontPointSizes['x-small'] = 1;
		fontPointSizes['small'] = 2;
		fontPointSizes['medium'] = 3;
		fontPointSizes['large'] = 4;
		fontPointSizes['x-large'] = 5;
		fontPointSizes['xx-large'] = 6;
	...
	if (!this.isChrome5) {
		html = html.replace(/\[url(.*?)\[\/url\]/gi, function (thisMatch, between) {
			...
			if (between.indexOf(']') == 0) {
				...
				return '<a href="' + decodedText + '" mybb_href="' + decodedText + '" >' + text + '</a>';
			}
				...
				return '<a href="' + decodedHref + '" mybb_href="' + decodedHref + '">' + text + '</a>';
			}
			else if (between.indexOf('=') == 0 && between.match(/=(.+?)\](.*)/)) {
				var href = RegExp.$1;
				var text = RegExp.$2;
	...
	//html = html.replace(/\[img=([0-9]{1,3})x([0-9]{1,3}) align=([a-z]+)\](\r\n?|\n?)(https?://([^<>\"']+?))\[\/img\]/gi, '<img src="sss $4" mybb_src="sss $4" width="$1" height="$2" alt="wysiwyg image" style="float:$3" />');
	html = html.replace(/\[img=([0-9]{1,3})x([0-9]{1,3}) align=([a-z]+)\]([^\[]+)\[\/img\]/gi, '<img src="$4" mybb_src="$4" alt="wysiwyg image" style="float: $3; width: $1px; height: $2px;" />');
	html = html.replace(/\[img align=([a-z]+)\]([^\[]+)\[\/img\]/gi, '<img src="$2" mybb_src="$2" alt="wysiwyg image" style="float:$1" />');
	html = html.replace(/\[img=([0-9]{1,3})x([0-9]{1,3})\]([^\[]+)\[\/img\]/gi, '<img src="$3" mybb_src="$3" alt="wysiwyg image" style="width: $1px; height: $2px;" />');
	
	// [img]src[/img] to <img src="src" mybb_src="src" alt="wysiwyg image" />
	html = html.replace(/\[img\]([^\[]+)\[\/img\]/gi, '<img src="$1" mybb_src="$1" alt="wysiwyg image" />');

	// [img='src']src[/img] to <img src="src" mybb_src="src" alt="" />
	html = html.replace(/\[img=(([^']+)|([^,\]]+))\](.|\n)*?\[\/img\]/gi, '<img src="$2$3" mybb_src="$2$3" alt="wysiwyg image" />');

	// [img='src',(right|left)]src[/img] to <img src="src" mybb_src="src" alt="wysiwyg image" style="float:(right|left)" />


	// [img=src]src[/img] to <img src="src" mybb_src="src" alt="wysiwyg image" />
	//html = html.replace(/\[img=(?!("|'))([^\]]+)\](.|\n)*?\[\/img\]/gi, '<img src="$1" mybb_src="$1" alt="wysiwyg image" />');
	...
	if (!tinyMCE.isOpera) {
		...
		// [quote='[clan]name'] => <blockquote quotedata="name" class="wysiwygQuote">
		//html = html.replace(/\[quote='([^']+)'\]/gi, '<br><blockquote quotedata="$1" class="wysiwygQuote container-4">');

		// [quote='[clan]name',url] => <blockquote quotedata="name" linkhref="url" class="wysiwygQuote">
		//html = html.replace(/\[quote='([^']+)',([^\]]+)\]/gi, '<br><blockquote quotedata="$1" linkhref="$2" class="wysiwygQuote container-4">');

		// [quote=name,url] => <blockquote quotedata="name" linkhref="url" class="wysiwygQuote">
		//html = html.replace(/\[quote=([^\,\]]+),([^\]]+)\]/gi, '<br><blockquote quotedata="$1" linkhref="$2" class="wysiwygQuote conainter-4">');

		// [quote=name] => <blockquote quotedata="name" class="wysiwygQuote">
		html = html.replace(/\[quote=([^\]]+)\]/gi, '<br><blockquote quotedata="$1" class="wysiwygQuote container-4">');
...
TinyMCE_Engine.prototype.htmlToBBCode = function (html) {
	...
	//code = code.replace(/<img([^>]+?|\\s+)height=["\']([^>]+?|\\s+)>/gi, '[img=$1x$2 align=$3]$5[\/img\]');
	code = code.replace(/<img src="(.+?)" alt="wysiwyg image" style="float: (left|center|right); width: ([0-9]{1,3})px; height: ([0-9]{1,3})px;" \/>/gi, '[img=$3x$4 align=$2]$1[\/img\]');
	code = code.replace(/<img src="(.+?)" alt="wysiwyg image" style="float: (left|center|right);" \/>/gi, '[img align=$2]$1[\/img\]');
	code = code.replace(/<img src="(.+?)" alt="wysiwyg image" style="width: ([0-9]{1,3})px; height: ([0-9]{1,3})px;" \/>/gi, '[img=$2x$3]$1[\/img\]');
	//code = code.replace(/<img([^>]+?|\\s+)>/gi, '[img=$1x$2 align=$3]$5[\/img\]');
	
	//code = code.replace(/\[img=([0-9]{1,3})x([0-9]{1,3}) align=([a-z]+)\]([^\[]+)\[\/img\]/gi, '<img src="$4" mybb_src="$4" alt="wysiwyg image" style="float:$3" width="$1" height="$2" />');
	//code = code.replace(/\[img align=([a-z]+)\]([^\[]+)\[\/img\]/gi, '<img src="$2" mybb_src="$2" alt="wysiwyg image" style="float:$1" />');
	//code = code.replace(/\[img=([0-9]{1,3})x([0-9]{1,3})\]([^\[]+)\[\/img\]/gi, '<img src="$3" mybb_src="$3" alt="wysiwyg image" width="$1" height="$2" />');
		...
		for (var i = 0; i < nodes.length ; i++) {
			...
			if (node.match(allowedHTMLOpenTags)) { 
				...
				switch (tagName) {
					...
					// Blockquote
					case 'blockquote':
						var quotedata = typeof(attributes['quotedata']) != 'undefined' ? '=' + attributes['quotedata'] + '' : '';
						replaceTag = '[quote' + quotedata + ']';
					break;
					...
					case 'li':
						...
						// Opera div with align and IE p with align
							...
							//if (decodeURI(escape(RegExp.$1)) == decodeURI(escape(attributes['mybb_href'])) || decodeURI(escape(RegExp.$1)) == decodeURI(escape(attributes['href']))) {
							if (RegExp.$1 == attributes['mybb_href'] || RegExp.$1 == attributes['href']) {
								replaceTag = '[url]';
							}
							// Link text differs. use attribute in bbcode tag
								...
								//var href = attributes['mybb_href'] ? decodeURI(attributes['mybb_href']) : decodeURI(attributes['href']);
								var href = attributes['mybb_href'] ? attributes['mybb_href'] : attributes['href'];
								replaceTag = "[url=" + href + "]";
							}
						}
		...
		switch (styleProperty.toLowerCase()) {
			...
			case 'float':
				if (tagName == 'img' && propertyValue.toLowerCase().match(/(left|right)/i)) {
					var src = attributes['mybb_src'] ? attributes['mybb_src'] : attributes['src'];
	...
	if (openNodes = tmpBBCode.match(/(\[[^\]]+\])/g)) {
		if (openNodes.length > 1) {
			...
			if (isImage && /\[img\]/.test(bbCode)) {
				return html.replace(/<img.*?src="(.*?)".*?(?:mybb_src="(.*?)")(?:.*)>/i, function (thisMatch, src, mybb_src) {
					if (typeof(mybb_src) == "undefined" || mybb_src == '') return bbCode + src + closingTags;
					else return bbCode + mybb_src + closingTags;
				});
			}
		...
		// No multiple tag found
			...
			if (isImage && /\[img\]/.test(bbCode)){
				return html.replace(/<img.*?src="(.*?)".*?(?:mybb_src="(.*?)")?(?:.*?)>/i, function (thisMatch, src, mybb_src) {
					if (typeof(mybb_src) == "undefined" || mybb_src == '') return bbCode + src + endTag;
					else return bbCode + mybb_src + endTag;
				});
			}
			else if (isImage && /\[img=/.test(bbCode)) {
...
/* Creates the popup div for the color picker */
TinyMCE_Engine.prototype.createVideoSelect = function(editorID, isSimpleArea) {
	var videoselectElement = document.getElementById(editorID + '_video');
	if (!videoselectElement) return;

	var videoselectDiv = document.createElement('div');
	videoselectDiv.id = editorID + '_videoMenu';
	videoselectDiv.className = 'hidden';
	
	// Defines the color picker colors
	var websites = new Array("Dailymotion=dailymotion","Google Video=googlevideo","MetaCafe=metacafe","MySpace TV=myspacetv","Vimeo=vimeo","Yahoo Video=yahoo","YouTube=youtube");
		
	// Builds inner HTML for color picker
	var videoSelectHTML = "";
	var url = tinyMCE.settings['imageURL'];
	videoSelectHTML += '<div class="mceVideo"><ul>';
	
	for (var i = 0; i < websites.length; i++) {
		var website = websites[i];
		
		var website = website.split("=");
		var command = "javascript:tinyMCE.execInstanceCommand('mce_editor_0','mcevideotag',false,'" + website[1] + "');"	
		videoSelectHTML += '<li><a href="' + command + '" title="' + website[0] + '">' + website[0] + '</a></li>';
		
	}
	videoSelectHTML += '</ul></div>';
	videoselectDiv.innerHTML = videoSelectHTML;
			
	// Adds color picker to the document and registeres the color picker icon as a popup menu
	var parentElement = videoselectElement.parentNode;
	parentElement.appendChild(videoselectDiv);
	popupMenuList.register(editorID + '_video');
}
	...
	content = content.replace(/<[^>]*?>/g, function(thisMatch) {
		thisMatch = thisMatch.replace(/ ([^=]+)="([^"]*)"/g, function(match2, attributeName, attributeValue) {
			if ((attributeName == "id" && attributeValue == 'pasteCursorPosition') 
				...
				|| attributeName == 'quotedata' || attributeName == 'linkhref' 
...
var Smilies = {
	init: function()
	{	
		 Smilies.bindSmilieInserter("clickable_smilies");
	},
	...
	insertSmilie: function(e)
	{
        ...
        tinyMCE.insertSmiley(element.src, element.alt, element.alt);
		//tinyMCE.execInstanceCommand('mce_editor_0', 'mceinsertcontent', false, '<img class="smilie" src="'+ element.src +'" alt="'+ element.alt +'">');
	},

	insertAttachment: function(aid)
	{
		//this.performInsert("[attachment="+aid+"]", "", true, false);
		tinyMCE.insertAttachment(aid);
	}
};
	
Event.observe(document, 'dom:loaded', Smilies.init);
