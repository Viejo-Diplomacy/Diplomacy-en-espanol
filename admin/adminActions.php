	...
	public static $actions = array(
			...
			'makeDonatorBronze' => array(
				...
				'params' => array('userID'=>'User ID'),
                            ...
                            'makeLigaParticipa' => array(
				'name' => 'Inscribe jugador en Liga',
				'description' => 'Marca a un jugador como participante de una liga o torneo (anyade un icono)',
				'params' => array('userID'=>'User ID'),
			),
			'makeLigaGanador1' => array(
				'name' => 'Nombra jugador ganador de Liga',
				'description' => 'Nombra a jugador ganador de liga o torneo (anyade un icono)',
				'params' => array('userID'=>'User ID'),
			),
...
>>>>>>> 4d9b181c65f726a9b242cf8e4ffd9c256e924d30
...

	{
		...
		return $this->makeDonatorType($params,'Bronze');
         ...
         private function makeLigaType(array $params, $type='') {
		global $DB;

		$userID = (int)$params['userID'];

		$DB->sql_put("UPDATE wD_Users SET type = CONCAT_WS(',',type,'Liga".$type."') WHERE id = ".$userID);

		return 'User ID '.$userID.' ha cambiado status liga.';
	}

	public function makeLiga(array $params)
	{
		return $this->makeLigaType($params);
	}
	public function makeLigaParticipa(array $params)
	{
		return $this->makeLigaType($params,'Participa');
	}
	public function makeLigaGanador1(array $params)
	{
		return $this->makeLigaType($params,'Ganador1');
